...
BOOST_AUTO_TEST_CASE(PartitionAlert)
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        indexDummy[i].nTime = now - (100-i)*params.TargetSpacing();
        // Other members don't matter, the partition check code doesn't
        // use them
    }
    chainDummy.SetTip(&indexDummy[99]);

    // Test 1: chain with blocks every params.TargetSpacing() seconds,
    // as normal, no worries:
    PartitionCheck(falseFunc, csDummy, chainDummy, params);
    ...
    SetMockTime(now);
    int64_t quickSpacing = params.TargetSpacing()*2/5;
